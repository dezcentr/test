  ...
  >
    <template v-slot:top>
      <div style="width: 100%" class="row">
        <div class="col-9">
          <q-btn v-if="['admin'].includes(user.getRole)" :to="{name: 'new-group'}">Добавить группу</q-btn>
        </div>
        <div class="col-3">
    ...
    <template v-slot:body-cell-action="props">
      <q-td :props="props">
        <q-btn :to="{name: 'edit-group', params: { id: props.row.id} }" label="Подробнее"/>
      </q-td>
    </template>
...
export default {
  ...
  methods: {
    ...
    onRequest(props) {
      this.loading = true
      api.get(process.env.API_GET_GROUP_ALL)
        .then(({data}) => {
          if(data.status === 200) {
