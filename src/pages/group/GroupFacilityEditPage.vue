<template>
  <q-page padding>
    <q-btn :to="{ name: 'group-facility', params: { group_id: group.id } }"
      >назад</q-btn
    >
    <h5>{{ group.name }} - {{ facility.name }} - Редактирование объекта</h5>
    ...
    <q-card class="q-pa-md">
      <q-form class="q-gutter-sm">
        ...
        <h6
          ...
          :class="{
            'groups-table-title--opened': isGroupsOpened,
          }"
        ...
        <div class="row">
          <div class="col-xs-12 col-sm-6 col-md-5 col-lg-3">
            ...
            <q-btn
              ...
              :to="{
                name: 'log-index',
                params: { facility_id: facility.id, ctx: groupID },
              }"
...
export default {
  name: "GroupFacilityEditPage",
  data() {
    const route = useRoute();
    const groupID = route.params.group_id ?? 0;
    const facilityID = route.params.id ?? 0;

    if (groupID === 0 || facilityID === 0) {
      this.$q.notify({
        type: "negative",
        message: "Ошибка при загрузки списка",
      });
      this.$router.push({ name: "group-facility" });
    }
  ...
  methods: {
    ...
    init: function () {
      ...
      this.groupStore.loadOnlyId(this.groupID).then((value) => {
        this.group = value;
      });
    },
  },
